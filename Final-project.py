...
class main:
    ...
    def __init__(self):
        ...
        self.fixed_image_path = os.path.join(
            os.getcwd(), "Abdominal_Data/Reference/refLung_001.dcm")
    ...
    def iterateMovingImages(self):
        ...
        moving_image_folder = os.path.join(
            os.getcwd(), "Abdominal_Data\\Moving")
        # iterating through all moving image and collecting their path
        for i in range(1, 201):
            self.moving_images.append(
                moving_image_folder + "\\Moving_" + f"{i:03}" + "\\Moving_" + f"{i:03}" + ".dcm")
    ...
    def segmentImage(self, moving_image_path, fixed_image_path):
        ...
        fixed_image2d = sitk.Extract(
            fixed_image, (fixed_image.GetWidth(), fixed_image.GetHeight(), 0), (0, 0, 0))
        moving_image = sitk.ReadImage(moving_image_path, sitk.sitkFloat32)
        moving_image2d = sitk.Extract(
            moving_image, (moving_image.GetWidth(), moving_image.GetHeight(), 0), (0, 0, 0))
        ...
        initial_transform = sitk.CenteredTransformInitializer(
            fixed_image2d, moving_image2d, sitk.Similarity2DTransform())
    ...
    def segmentAllImages(self):
        ...
        for i in self.iterateMovingImages():
            image_path = resampled_image_path + \
                "\\resampled" + i[-7:-4] + ".png"
            sitk.WriteImage(sitk.Cast(sitk.RescaleIntensity(self.segmentImage(
                i, self.fixed_image_path)), sitk.sitkUInt8), image_path)
        return None
    ...
    def generateVideo(self):
        ...
        out = cv2.VideoWriter('final-project.mp4',
                              cv2.VideoWriter_fourcc(*'mp4v'), 8, size)
